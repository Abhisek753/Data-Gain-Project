...
const Table = () => {
  ...
  return (
    <div className="w-full md:w-[98%] p-4 md:p-6 bg-white border rounded-lg shadow-lg relative h-auto md:h-[95vh]">
      ...
      <div className="">
        <table className="min-w-full text-left table-fixed">
          <thead>
            <tr>
              <th className="px-2 md:px-4 py-2">Donor</th>
              <th className="px-2 md:px-4 py-2">Panels</th>
              <th className="px-2 md:px-4 py-2">Barcode</th>
              <th className="px-2 md:px-4 py-2">Source</th>
              <th className="px-2 md:px-4 py-2">Date</th>
              <th className="px-2 md:px-4 py-2">Amount($)</th>
              <th className="px-2 md:px-4 py-2">Observed By</th>
          ...
          <tbody>
            {tableData.map((row, index) => (
              <tr key={index} className={`${
                ...
                <td className="px-2 md:px-4 py-2 text-teal-500 underline cursor-pointer">
                  {row.donor}
                </td>
                <td className="px-2 md:px-4 py-2">{row.panels}</td>
                <td className="px-2 md:px-4 py-2 text-teal-500 underline cursor-pointer">
                  {row.barcode}
                </td>
                <td className="px-2 md:px-4 py-2">{row.source}</td>
                <td className="px-2 md:px-4 py-2">{row.date}</td>
                <td className="px-2 md:px-4 py-2">{row.amount}</td>
                <td className="px-2 md:px-4 py-2">{row.observedBy}</td>
                <td className="px-2 md:px-4 py-2">{row.status}</td>
                <td className="px-2 md:px-4 py-2 relative">
                  ...
                  {openDropdownIndex === index && (
                    <div className="absolute mt-2 right-0 bg-white border rounded shadow-md z-50">
                      <button
